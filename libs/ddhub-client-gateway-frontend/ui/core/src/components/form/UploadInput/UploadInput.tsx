...
import { TextField, FileUploadPartial } from '@ddhub-client-gateway-frontend/ui/core';
...
export const UploadInput: FC<UploadInputProps> = (props) => {
  ...
  const { open, getRootProps, getInputProps, isValidFileType } = useUploadInputEffects(props);

  const chooseFileBox = (
    <>
      <Upload className={clsx(classes.icon, {
        [classes.iconDisabled]: props.isUploadDisabled,
      })}></Upload>
      <Typography variant='body2' paddingTop={4.6} className={clsx(classes.subtitle, {
        [classes.subtitleDisabled]: props.isUploadDisabled,
      })}
      ><strong>Drag and drop</strong> or browse to choose a file</Typography>
    </>
  );
  ...
  const invalidFileBox = (
    <>
      ...
      <Typography variant='body2' className={classes.subtitle} paddingTop={3}><strong>Upload failed:</strong> File not allowed</Typography>
    </>
  );
  ...
  const fileUploadedView = (
    <>
      ...
      <Typography variant='body2' className={classes.subtitle} marginTop={-0.625}>{props.value}</Typography>
      <Typography variant='body2' className={classes.subtitle} paddingTop={2.875} id='upload-message'>Upload complete!</Typography>
      <Typography variant='body2' className={classes.subtitle} paddingTop={2.875} id='replace-message'><strong>Drag and drop</strong> or browse to replace this file</Typography>
    </>
  );
