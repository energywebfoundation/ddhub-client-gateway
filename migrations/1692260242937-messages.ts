...
CREATE TABLE received_messages (
    ...
    FOREIGN KEY (topicId, topicVersion) REFERENCES topic(id, version) ON DELETE SET NULL
);

CREATE TABLE received_messages_mapping (
    fqcn VARCHAR(255) PRIMARY KEY,
    messageId VARCHAR(255) REFERENCES received_messages(messageId)
);

CREATE TABLE received_messages_read_status (
    messageId VARCHAR(255) REFERENCES received_messages(messageId),
...
CREATE TABLE sent_messages_recipients (
    clientGatewayMessageId VARCHAR(255) REFERENCES sent_messages(clientGatewayMessageId),
    messageId VARCHAR(255) PRIMARY KEY,
