...
export const GatewayIdentityContainer = ({ identity, enrolment, auth }: GatewayIdentityContainerProps) => {
  ...
  const handleCreate = async (privateKey?: string) => {
    ...
    try {
      ...
      swal('Success', 'Private key saved. If not already funded, visit https://voltafaucet.energyweb.org', 'success')
    } catch (err) {
